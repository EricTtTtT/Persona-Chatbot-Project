...
from Chatbot import *
from remove_duplicate_persona import *
...
def analyze():
    ...
    with torch.no_grad():
        count = 0
        total_rw = {}
        persona_pool = remove_duplicate_persona(threshold = 0.7)
        for input_ids, mc_token_ids, lm_labels, mc_labels, token_type_ids in tqdm(
            ...
            count += 1
            for i in range(10):
                for i_p, persona_i in enumerate(persona_pool):
                    ...
                    for i in range(args.batch_size):
                        ...
                        total_rw[tokenizer.decode(history_enc[i][0])][persona_i].append(score[i])
            ...
            if count == 3:
                ...
                print(total_rw)
                import json
                with open(f"All_total.json", "w") as file:
                    json.dump(total_rw, file)
                with open("All_score_05.json", "w") as file:
