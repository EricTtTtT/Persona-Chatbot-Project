...
def generate_response(personality, history, tokenizer, model, arg, current_output=None):
    ...
    sequence_bt = [
        [seq[0]] + [[speaker2 if (len(seq) - i) % 2 else speaker1] + s for i, s in enumerate(seq[1:])] for seq in sequence_bt
    ]
    ...
    sequence_bt = pad_sequence(
        [torch.LongTensor(x) for x in sequence_bt], batch_first=True, padding_value=tokenizer.encode("<pad>")[0]
    ).to(arg.device)
    token_type_ids_bt = pad_sequence(
        [torch.LongTensor(x) for x in token_type_ids_bt], batch_first=True, padding_value=speaker1
    ).to(arg.device)
...
def main():
    ...
    for i_epoch in range(args.epoch):
        ...
        for input_ids, mc_token_ids, lm_labels, mc_labels, token_type_ids in tqdm(train_loader):
            ...
            with torch.no_grad():
                response_enc = generate_response(persona_enc, history_enc, tokenizer, model, arg)
                for i in range(args.batch_size):
                    history_enc[i].append(response_enc[i])
            ...
            for r in rewards_ori:
                rewards.append((r - r.mean()) / (r.std() + 1e-9))
            ...
            if i_batch % args.save_time_step == 0:
                torch.save(
                    persona_selector, os.path.join(args.work_space, args.save_dir, args.model_name, f"{i_epoch}_epoch.pkl")
                )
