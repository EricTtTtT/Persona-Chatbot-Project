...
def main():
    ...
    parser.add_argument("--output_csv", type=str, default="positive_each_persona.csv")
    ...
    header = ["persona_id", "persona", "engaging", "coherence", "positive", "sum"]
    ...
    for i_persona, bot_persona in enumerate(persona_pool):
        ...
        with open(args.output_csv, "a") as f:
            ...
            for inter_persona_ori, history_ori, len_p, len_h in tqdm(train_loader):
                ...
                positive_score = goemotions.get_positive_score(bot_reply)
                coherence = get_coherence_score(last_history, bot_reply)
                engaging_score_sum += sum(score) / len(score)
                positive_score_sum += sum(positive_score) / len(positive_score)
            ...
            positive_mean = positive_score_sum / num_batch
            writer.writerow([i_persona, bot_persona, engaging_mean, coherence_mean, positive_mean, engaging_mean + coherence_mean + positive_mean])
