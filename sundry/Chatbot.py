...
def main():
    ...
    # load_path=args.load_model_path, bert_model = bert_model, lr = args.lr
    # )
    print(np.shape(persona_pool))
    # exit(0)
    ...
    for i_epoch in range(1):
        ...
        for input_ids, mc_token_ids, lm_labels, mc_labels, token_type_ids in tqdm(train_loader):
            ...
            if args.select:
                ...
                persona_s2, log_prob, _, critic_value_0 = (
                    random.sample(persona, args.batch_size),
            ...
            if not args.fix:
                if args.select:
                    ...
                    persona_s4, log_prob, _, critic_value_1 = (
                        random.sample(persona_pool, args.batch_size),
                        [0 for _ in range(args.batch_size)],
                    )
            else:
