...
class ActorCritic(nn.Module):
    ...
    def evaluate(self, action, **state):
        ...
        output = self.actor(**state)
        # last_layer_output = output[0].mean(1)
        action_probs = F.softmax(output[0], dim = -1)
        dist = Categorical(action_probs)
...
class PPO:
    ...
    def update_writer(self, writer, i_iter=0, turn=2, entropy_cof=0.01):
        ...
        for _ in range(self.K_epochs):
            ...
            # final loss of clipped objective PPO
            # loss = torch.min(surr1, surr2) - (self.entropy_cof * dist_entropy.sum())
            loss = -torch.min(surr1, surr2) - entropy_cof * dist_entropy.sum()
