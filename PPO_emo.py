...
class PPO:
    def __init__(self, persona_pool, bert_model, state_dim = 1608, action_dim = 6732, lr_actor = 0.00002, lr_critic = 0.05,
        ...
        self.output_dir = f"t_lra_{lr_actor}_lrc_{lr_critic}_K_{K_epochs}_eps_{eps_clip}_cri_{self.critic_cof}_entr_{self.entropy_cof}"
    ...
    def update_writer(self, writer, i_iter=0, turn=2):
        ...
        rewards = ((rewards - rewards.mean()) / (rewards.std() + 1e-7)) + rewards.mean()
        ...
        for _ in range(self.K_epochs):
            ...
            if np.random.rand() < 0.02:
                ...
                print("Advantage is ", advantages.sum())
                print("Rewards is ", rewards.sum())
                print("state value ", state_values.sum())
                print("critic loss : ", critic_loss)
